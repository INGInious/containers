 ...
 */
public class MyVisitor2 <T> extends AbstractParseTreeVisitor<T> implements PsiCoderVisitor<T> {
	...
	MyVisitor2(){
		...
		type.put("entero", Symbol.ENTERO);
		type.put("real", Symbol.REAL);
		type.put("cadena", Symbol.CADENA);
		type.put("caracter", Symbol.CARACTER);
		type.put("booleano", Symbol.BOOLEANO);
	...
	@Override public T visitDeclaracion(PsiCoderParser.DeclaracionContext ctx) { 
		...
		symbol.type = type.get(ctx.tipo().toString());
		
		visitTipo(ctx.tipo());
		
		symbol.name = (String)visitIdentificadorDeclaracion(ctx.identificadorDeclaracion());
